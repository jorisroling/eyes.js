...
eyes.inspector = function (options) {
    ...
    return function (obj, label, opts) {
        ...
        result=that.inspect.call(that, obj, label, myopts);
        if (myopts.html && !myopts.stream) {
            return '<pre class="eyes" style="padding:8px;background-color: black;color:'+myopts.styles.all+';">'+result+'</pre>';
        } else {
            return result;
        }
    };
...
result = (function (obj) {
        switch (typeOf(obj)) {
            ...
            case "string"   : obj = stringifyString((obj.indexOf("'") === -1 && !options.json)? ("'" + obj.replace(/'/g,"\\'") + "'") : ('"' + obj.replace(/"/g,'\\"') + '"'));
...
//        if (obj.hasOwnProperty && obj.hasOwnProperty(k) && !(obj[k] instanceof Function && options.hideFunctions)) {
        if (Object.prototype.hasOwnProperty.call(obj,k) && !(obj[k] instanceof Function && options.hideFunctions)) {

            out.push((options.json?'"':'')+eyes.stylize(quote_reserved_words(k), options.styles.key, options) + (options.json?'"':'') + ': ' +
